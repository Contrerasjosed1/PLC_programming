...
class WorkspaceView(QGraphicsView):
    ...
    # ------------------------------------------------------------------
    # Crear bloques según tipo (símbolos ladder)
    # ------------------------------------------------------------------
    def create_block(self, row: int, index: int, block_type: str):
        ...
        - "XIO": contacto NC
        - "OTE": bobina
        - "OTL": bobina latch (SET)
        - "OTU": bobina unlatch (RESET)
        - "TON": timer ON delay
        """
        if block_type == "XIC":
            item = self.create_contact_no_block()
            label_text = "XIC"
        elif block_type == "XIO":
            item = self.create_contact_nc_block()
            label_text = "XIO"
        elif block_type == "OTE":
            item = self.create_coil_block("OTE")
            label_text = "OTE"
        elif block_type == "OTL":
            item = self.create_coil_block("OTL")
            label_text = "OTL"
        elif block_type == "OTU":
            item = self.create_coil_block("OTU")
            label_text = "OTU"
        elif block_type == "TON":
            item = self.create_timer_block()
            label_text = "TON"
        else:
            item = self.create_contact_no_block()
            label_text = "BLK"
    ...
    # --- Constructores de símbolos ladder ---
    def create_contact_no_block(self):
        """
    ...
    def create_contact_nc_block(self):
        ...
        # Línea diagonal desde arriba de la izquierda hacia abajo de la derecha
        diag = scene.addLine(
            cx - offset, y1,
            cx + offset, y2,
            pen
    ...
    def create_coil_block(self, coil_type="OTE"):
        """
        OTE/OTL/OTU - Bobina:
        ...
        # Texto interno pequeño (OTE, OTL, OTU)
        inner_text = scene.addText(coil_type, QFont("Segoe UI", 7, QFont.Bold))
    ...
    def create_timer_block(self):
        ...
        # Pequeño rectángulo interno tipo "módulo"
        margin = 10
        inner = scene.addRect(
            margin, margin,
    ...
    def delete_block(self, block):
        """Elimina un bloque de la escena y de la estructura rows."""
        row = block.data(1)
        if row is not None:
            row = int(row)
            if 0 <= row < len(self.rows):
                row_list = self.rows[row]
                if block in row_list:
                    row_list.remove(block)
                    self.layout_row(row)
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.setWindowTitle("PLC Designer - Ladder con símbolos")
